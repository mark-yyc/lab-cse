        ...
        int to_set, struct fuse_file_info *fi)
{
    ...
    if ((FUSE_SET_ATTR_SIZE & to_set)||(to_set & FUSE_SET_ATTR_ATIME)||(to_set & FUSE_SET_ATTR_MTIME)) {
        ...
        off_t off, struct fuse_file_info *fi)
{
    ...
    yfs->readdir(inum, entries);
    for (std::map<std::string,yfs_client::inum>::iterator it=entries.begin(); it!=entries.end(); ++it){
        dirbuf_add(&b, it->first.c_str(), (fuse_ino_t) it->second);
    }
