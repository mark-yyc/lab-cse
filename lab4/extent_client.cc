...
extent_client::extent_client(std::string dst)
{
  ...
  rlsrpc->reg(rextent_protocol::pull, this, &extent_client::pull_handler);
  rlsrpc->reg(rextent_protocol::push, this, &extent_client::push_handler);

  file_cache[1] = new file_info(true, true, extent_protocol::T_DIR);    
}
...
extent_client::create(uint32_t type, extent_protocol::extentid_t &eid)
{
  ...
  file_cache[eid] = new file_info(false, false, type);
  return ret;
}
...
extent_client::get(extent_protocol::extentid_t eid, std::string &buf)
{
  ...
  if (!file_cache.count(eid)) {
    ...
    file_cache[eid] = new file_info(false, true, 0);
		       ...
		       extent_protocol::attr &attr)
{
  ...
  if (!file_cache.count(eid)) {
    ...
    file_cache[eid] = new file_info(true, false, attr.type);
...
rextent_protocol::status
extent_client::push_handler(extent_protocol::extentid_t eid, int &) {
  int r;
  cl->call(extent_protocol::put, id, eid, file_cache[eid]->buf, r);
  if (file_cache.count(eid)) {
    file_cache[eid]->attr.atime = file_cache[eid]->attr.mtime = file_cache[eid]->attr.ctime = time(NULL);
    file_cache[eid]->buf_dirty = file_cache[eid]->attr_dirty = false;
  }
